...
public class LiDARTrackingViewer {
	...
	public LiDARTrackingViewer(String dataset, ViewerType viewType, Properties conf) {
	...
	public static void main(String[] args) {
		...
		if(args[1].equals("-d")){
			...
			SwingUtilities.invokeLater(new Runnable() {
				@Override
				public void run() {
					new LiDARTrackingViewer(filename, viewType, null);  // run the constructor
				}
			});
			...
			try{
				...
				SwingUtilities.invokeLater(new Runnable() {
					@Override
					public void run() {
						new LiDARTrackingViewer(filename, viewType, conf);  // run the constructor
					}
				});
			}catch(Exception e){
				e.printStackTrace();
