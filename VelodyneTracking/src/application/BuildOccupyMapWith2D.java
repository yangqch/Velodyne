...
import velodyne2d.LidarFramePorcessor2D;
...
public class BuildOccupyMapWith2D {
	...
	public static void buildOccupyGridMap(String low_level_data_filename, String mid_level_data_filename, String occupy_map_dir, double conn_comp_neighbor_thres, int conn_comp_size_thres, 
		...
		try{
			...
			LidarFramePorcessor2D mid2d = new LidarFramePorcessor2D(midProcessor);
			while(true){
...
//				Point3D[] w_midPoints = trans.transform4D(localWorldFrame, null, midProcessor.getVirtualTable().getPoint3D(midProcessor.getCompFilter().getCompMask(), false));
				Point2D[] mid_points2d = mid2d.getScan().getPoints2D(null);
				Point3D[] mid_points3d = new Point3D[mid_points2d.length];
				for(int i=0; i<mid_points2d.length; i++){
					Point2D p = mid_points2d[i];
					mid_points3d[i] = new Point3D(p.x, p.y, 0);
				}
				Point3D[] w_midPoints = trans.transform4D(localWorldFrame, null, mid_points3d);
